    ...
    return (
        <>
            <div className={`${styles.Pilier} ${(active == title) ? undefined : (active != '' && active) ? styles.NoActive : undefined}`}>
                ...
                {link ? (
                    <Link href={link} className="btn__normal btn__dark">Découvrir les rencontres</Link>
                ) : (
                    <button onClick={(event) => setFiltres(prev => { return { ...prev, nom: '', pilier: title } })} className="btn__small btn__dark">Découvrir les rencontres</button>
                )}
            </div>
        </>
    )
