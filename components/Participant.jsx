...
export default function Participant({ data, setActions, session }) {
  ...
  const [role, setRole] = useState((data.role || 'participant').toLowerCase())

  // 🔄 Synchroniser le rôle avec les nouvelles props si filtré
  useEffect(() => {
    setRole((data.role || 'participant').toLowerCase())
  }, [data.role])

  const deleteUser = async () => {
    setAlert(null)
    await fetch(`/api/registrations/delete?userId=${data.userId ? data.userId : data.accountId}&sessionId=${data.sessionId}&${!data.userId && 'specialAccount=true'}`, { method: 'DELETE' })
  ...
  return (
    <>
      <div className={`${styles.User} ${data.deleted ? styles.Canceled : undefined}`}>
        <div className="flex aligncenter gap10">
          <span className={styles.UserName}>
            <span className="material-icons">{data.deleted ? 'cancel' : 'person'}</span>
            {data.nom} {data.prenom}
          </span>
          {!data.deleted && (
            <button onClick={preDeleteUser} className={styles.Corb}>
              <span className="material-icons">delete</span>
            </button>
          )}
          <button onClick={generateBadge} className={styles.Corb}>
        ...
        <div className={styles.Table}>
          {data.nom ? (
            <>
              ...
              <div className="w20"><span className={styles.Label}>Téléphone</span>{data.telephone || '-'}</div>
              ...
              <div className="w20"><span className={styles.Label}>Ville</span>{data.ville || '-'}</div>
              ...
              <div className="w20"><span className={styles.Label}>Repas</span>
                {data.repas ? 'J1 : Oui' : 'J1 : Non'}
                {data.session.metasSession.nombreJours == 2 ? data.repas2 ? ' / J2 : Oui' : ' / J2 : Non' : ''}
                {data.regime ? ` (${data.regime})` : ''}
              </div>
              <div className="w30"><span className={styles.Label}>Besoin spécifique</span>{data.besoins || '-'}</div>
      ...
      {notif && <Notif datas={notif} setNotif={setNotif} />}
      {alert && <Alert datas={alert} setAlert={setAlert} />}
    </>
  )
}
